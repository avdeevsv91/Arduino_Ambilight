/*
   ...
   Доработал AvdeevSV https://io-net.ru/samodelnyj-ambilight-dlya-monitora-kompyutera/
   2020
*/
//----------------------НАСТРОЙКИ-----------------------
#define NUM_LEDS 83          // число светодиодов в ленте
#define DI_PIN 13            // пин, к которому подключена лента
#define OFF_TIME 10          // время (секунд), через которое лента выключится при пропадании сигнала
#define OFF_SMOOTH 1000      // плавное отключение (-1 выкл, больше или равно нулю - длительность (мкс) каждого уровня яркости)
...
void check_connection() {
  if (led_state) {
    if (millis() - off_timer > (OFF_TIME * 1000)) {
      led_state = false;
      if (OFF_SMOOTH >= 0) {
        new_bright_f = LEDS.getBrightness();
        for (i = new_bright_f; i > 0; i--) {
          new_bright_f = new_bright_f * COEF + i * (1 - COEF);
          LEDS.setBrightness(new_bright_f);
          FastLED.show();
          delayMicroseconds(OFF_SMOOTH);
        }
      }
      FastLED.clear();
      FastLED.show();
...
void loop() {
  if (AUTO_BRIGHT && led_state) {            // если включена адаптивная яркость и лента активна
